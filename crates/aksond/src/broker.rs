...
use akson_authority::{AttemptState, CapabilityComponent, Grant};
use akson_broker::{
...
use akson_store::{PrepareOutcome, Store};
use akson_transport::tls::{ca_client_config, client_config};
...
mod tests {
    use super::*;
    use akson_broker::{AuthScheme, Origin, ProcessorConfig};
    ...
    fn store() -> StdMutex<Store> {
        let kek = akson_store::envelope::Kek::from_bytes([51u8; 32]);
    ...
    fn processor(store: &StdMutex<Store>) -> (ProcessorConfig, EgressPolicy) {
        let config = ProcessorConfig {
            ...
            disclosure: akson_broker::Disclosure::remote("Local", "here"),
            path: "/".to_owned(),
    ...
    fn ident(agent: &str) -> akson_contract::Identity {
    ...
    fn daemon() -> DaemonState {
        ...
        .unwrap();
        let kek = akson_store::envelope::Kek::from_bytes([52u8; 32]);
        let cp = ExternalCheckpoint {
            state_generation: 0,
            trusted_time: 0,
            rollback_detectable: true,
        ...
        let config = DaemonConfig {
            data_dir: std::env::temp_dir().join("aksond-broker-unused"),
    ...
    fn work_order(id: &str, grants: Vec<Grant>) -> WorkOrder {
        WorkOrder {
            ...
            audience: Audience {
                ...
                executor: "akson-worker".to_owned(),
            },
    ...
    fn a_processor_call_for_a_different_processor_is_refused() {
        ...
        store_work_order(
            &daemon,
            "wo-1",
            vec![Grant::ProcessorUse(akson_authority::ProcessorUseScope {
