...
impl IdentityKeys {
    ...
    /// secret master: `SHA-256("akson/identity-key/v1/" ‖ label ‖ 0x00 ‖ master)`.
    fn derive(&self, label: &str) -> [u8; 32] {
        let mut h = Sha256::new();
        h.update(b"akson/identity-key/v1/");
...
mod tests {
    ...
    fn the_endpoint_key_produces_a_self_signed_cert() {
        ...
        let cert =
            self_signed_endpoint(&endpoint, "akson-endpoint", Duration::from_secs(3600)).unwrap();
