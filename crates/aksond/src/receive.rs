...
//! 2. **Validation** (§10.2) — [`receive_proposal`](akson_contract::receive_proposal)
...
use akson_contract::{expires_at_unix, receive_proposal, Identity, PartBody, ReceivedProposal};
use akson_crypto::keypair::PurposeVerifyingKey;
use akson_proto::v1::{Part, Task, TaskState, TaskStatus};
...
pub fn dispatch_proposal(
    ...
    store.submit_revision(&task_id, &received.proposal, &covered.peer, expires, trusted_now_unix)?;
...
fn persist_worker_inputs(
    ...
    for (ordinal, entry) in received.proposal.contract.inputs.iter().enumerate() {
        ...
        let bytes = match &part.body {
            ...
            PartBody::Data(v) => match akson_ext::jcs::canonical_bytes(v) {
                Ok(b) => b,
                Err(_) => continue,
            },
        };
...
mod tests {
    ...
    fn store() -> Store {
        let kek = akson_store::envelope::Kek::from_bytes([5u8; 32]);
    ...
    fn message(key: &PurposeKey) -> Vec<Part> {
        let env = akson_contract::sign_proposal(&contract_payload(), key).unwrap();
    ...
    fn a_valid_proposal_becomes_a_submitted_task_and_is_persisted() {
        ...
        assert!(matches!(
            ...
            akson_contract::HeadState::Open(_)
        ));
