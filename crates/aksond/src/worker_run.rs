...
//!
//! `akson task run <task_id>` composes the spine end to end for a Task that has
...
//!   manifest (the same object `akson task deliver` then sends to the requester).
//!
//! The worker is the operator's command (`AKSON_WORKER_CMD`), run as
...
use akson_authority::AttemptEvent;
...
///
/// - `Shell` — the operator's `AKSON_WORKER_CMD`, run as `/bin/sh -c <cmd>` under the
...
/// - `Adapter` — `AKSON_WORKER_EXEC` as a direct `argv` (no shell) under the strict
...
pub fn run_worker(state: &DaemonState, task_id: &str) -> Result<serde_json::Value, Problem> {
    ...
    // AKSON_BROKER_FD; the daemon makes the real, credential-injected, budgeted call.
    let mut broker = if confinement.processor.is_some() {
        ...
        spec = spec.setenv("AKSON_BROKER_FD", &worker_fd.as_raw_fd().to_string());
...
mod tests {
    ...
    fn collect_artifacts_records_declared_artifacts_and_tolerates_none() {
        let dir = std::env::temp_dir().join(format!("akson-wr-a-{}", std::process::id()));
        ...
        let empty = std::env::temp_dir().join(format!("akson-wr-e-{}", std::process::id()));
    ...
    fn a_non_inert_artifact_is_refused() {
        let dir = std::env::temp_dir().join(format!("akson-wr-inert-{}", std::process::id()));
        let artifacts = dir.join("artifacts");
        std::fs::create_dir_all(&artifacts).unwrap();
        std::fs::write(
            dir.join("artifacts.json"),
    ...
    fn a_declared_but_unwritten_artifact_is_an_error() {
        let dir = std::env::temp_dir().join(format!("akson-wr-m-{}", std::process::id()));
    ...
    fn a_malicious_artifact_role_cannot_escape_the_output_dir() {
        let dir = std::env::temp_dir().join(format!("akson-wr-esc-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
