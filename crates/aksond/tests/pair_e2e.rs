...
use aksond::{run_pair_accept, ControlRequest, DaemonConfig, DaemonState, IdentityKeys};
use serde_json::json;
...
async fn a_daemon_accepts_an_invitation_and_pins_the_inviter_with_its_keys() {
    ...
    let config = DaemonConfig {
        data_dir: std::env::temp_dir().join("aksond-pair-unused"),
...
async fn two_daemons_pair_via_the_daemon_bootstrap_endpoint() {
    use akson_transport::bootstrap::{serve as serve_bootstrap, BootstrapState};
    use aksond::{run_pair_invite, SharedStore};
    ...
    let a_config = DaemonConfig {
        data_dir: std::env::temp_dir().join("aksond-pair-a-unused"),
        local_performer: Identity {
            issuer: "iss".to_owned(),
    ...
    let b_config = DaemonConfig {
        data_dir: std::env::temp_dir().join("aksond-pair-b-unused"),
        local_performer: Identity {
            issuer: "iss".to_owned(),
            agent: "accepter".to_owned(),
        },
