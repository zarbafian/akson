...
//! What you write:
//! ```
//! use axon_authority::{evaluate, PolicyDecision, StandingRule};
//! use axon_contract::Identity;
//! let peer = Identity { issuer: "iss".into(), agent: "requester".into() };
//! let rules = vec![StandingRule::deny(peer.clone(), Some("https://axon.invalid/spam".into()))];
//! // A denied task type is refused without a prompt; anything else prompts.
//! assert_eq!(evaluate(&rules, &peer, "https://axon.invalid/spam"), PolicyDecision::Deny);
//! assert_eq!(evaluate(&rules, &peer, "https://axon.invalid/review"), PolicyDecision::Prompt);
//! ```
...
mod tests {
    ...
    fn binding() -> RuleBinding {
        RuleBinding {
            ...
            contract_task_type: "https://axon.invalid/review".to_owned(),
    ...
    fn deny_rule_matches_by_subject_and_task_type() {
        let rules = vec![StandingRule::deny(
            peer("spammer"),
            Some("https://axon.invalid/spam".to_owned()),
        )];
        assert_eq!(
            evaluate(&rules, &peer("spammer"), "https://axon.invalid/spam"),
            PolicyDecision::Deny
        );
        // Different task type from the same peer is not denied by this rule.
        assert_eq!(
            evaluate(&rules, &peer("spammer"), "https://axon.invalid/review"),
            PolicyDecision::Prompt
        );
        // Different peer is not denied.
        assert_eq!(
            evaluate(&rules, &peer("other"), "https://axon.invalid/spam"),
            PolicyDecision::Prompt
        );
    }
    ...
    fn deny_all_task_types_from_a_subject() {
        ...
        assert_eq!(
            evaluate(&rules, &peer("blocked"), "https://axon.invalid/anything"),
            PolicyDecision::Deny
        );
    }
    ...
    fn no_rule_prompts() {
        assert_eq!(
            evaluate(&[], &peer("anyone"), "https://axon.invalid/x"),
            PolicyDecision::Prompt
        );
    }
