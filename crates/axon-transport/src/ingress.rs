...
//! What you write:
//! ```no_run
//! # use axon_transport::ingress::{admit, Admit, Ingress};
//! # use std::collections::BTreeSet;
//! # fn go(store: &axon_store::Store, req: &Ingress) {
...
use axon_proto::{profile, A2A_MEDIA_TYPE, A2A_VERSION};
use axon_store::delivery::{body_digest, verify_content_digest, CoveredValues, DeliveryError};
...
mod tests {
    ...
    fn rejects_missing_required_extension() {
        ...
        required.insert("https://axon.invalid/ext/contract/v1".to_owned());
        assert!(matches!(
