...
//! let cert = self_signed_endpoint(&key, "akson-endpoint", Duration::from_secs(86_400)).unwrap();
...
mod tests {
    ...
    fn generates_self_consistent_cert() {
        let key = tls_key();
        let cert =
            self_signed_endpoint(&key, "akson-endpoint", Duration::from_secs(86_400)).unwrap();
        assert!(cert.pem.starts_with(b"-----BEGIN CERTIFICATE-----"));
    ...
    fn wrong_purpose_fails_closed() {
        ...
        assert!(matches!(
            self_signed_endpoint(&card, "akson-endpoint", Duration::from_secs(60)),
