...
//! AXON_WAN_PEER_ADDR=10.0.0.2:18444 \
//! AXON_WAN_PEER_CERT=<sha-256 hex of the performer's endpoint cert> \
//! AXON_WAN_REQUESTER=orgA/alice \
//! AXON_WAN_PERFORMER=orgB/bob \
//! AXON_WAN_EXCHANGES=10 \
//!   cargo test -p axond --test wan_keepalive -- --ignored --nocapture
//! ```
...
fn exchange_body(
    ...
    let value = json!({
        ...
        "requester": {"issuer": requester.0, "agent": requester.1},
        "performer": {"issuer": performer.0, "agent": performer.1},
...
async fn many_exchanges_share_one_connection_to_a_remote_peer() {
    let data_dir = env("AXON_WAN_DATA_DIR").expect("set AXON_WAN_DATA_DIR");
    let peer_addr = env("AXON_WAN_PEER_ADDR").expect("set AXON_WAN_PEER_ADDR (host:port)");
    let peer_cert = env("AXON_WAN_PEER_CERT").expect("set AXON_WAN_PEER_CERT (sha-256 hex)");
    let requester = identity(&env("AXON_WAN_REQUESTER").unwrap_or_else(|| "orgA/alice".into()));
    let performer = identity(&env("AXON_WAN_PERFORMER").unwrap_or_else(|| "orgB/bob".into()));
    let exchanges: usize = env("AXON_WAN_EXCHANGES")
        .unwrap_or_else(|| "10".into())
        .parse()
        .expect("AXON_WAN_EXCHANGES must be a number");
    ...
    let client_cfg =
        axon_transport::tls::client_config(&tls_key, &our_cert, &peer_fingerprint).unwrap();
