//! Two agents, two components, many rounds: a cooperation scenario over Axon.
//!
...
//! [`UiAgent`]): Axon carries the delegation and the evidence, the agent supplies
...
/// `axon task send` → `axon task approve` → `axon task run` → `axon task deliver`
/// → `axon task output`.
async fn exchange(
    requester: &Endpoint,
    performer: &Endpoint,
    ...
    let spec = TaskSpec {
        ...
        task_type: "https://axon.invalid/task/component-change/v1".to_owned(),
    ...
    // 5. The requester reads the result back — `axon task output <id>`.
    let read = requester
        .state
...
fn pin_peer(local: &Endpoint, peer: &Endpoint) {
    ...
    store
        .put_peer(&axon_store::StoredPeer {
