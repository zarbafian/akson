...
use axon_pairing::invitation::{Invitation, PendingInvitation};
use axon_pairing::key_binding::KeyBindingSet;
...
/// (`AXON_PAIR_ADDR`), since the invitation must carry that reachable endpoint URL.
pub fn run_pair_invite(state: &DaemonState) -> Result<serde_json::Value, Problem> {
    let pair_addr = state.config().pair_addr.as_ref().ok_or_else(|| {
        problem(
            409,
            "pairing-not-enabled",
            "the pairing endpoint is not configured (set AXON_PAIR_ADDR)",
        )
    })?;
