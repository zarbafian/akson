...
//!
//! Axon defines only the minimal predicate *types*; the predicate body is the
...
//! What you write:
//! ```
//! use axon_evidence::{Statement, Subject, PREDICATE_EXECUTION_V1};
...
use axon_crypto::purpose::KeyPurpose;
use axon_ext::dsse::{self, Envelope};
use axon_ext::jcs;
...
/// type). Distinct from Axon's own `vnd.axon-dev.*` types (§14.2).
pub const INTOTO_PAYLOAD_TYPE: &str = "application/vnd.in-toto+json";

/// Axon's authorization predicate type (request digest, issuer, capability vector,
/// policy decision, executor audience — §14.2).
pub const PREDICATE_AUTHORIZATION_V1: &str = "https://axon.invalid/attestation/authorization/v1";
/// Axon's execution predicate type (materials, processor/runner/sandbox identity,
/// outputs, resource use, terminal state — §14.2).
pub const PREDICATE_EXECUTION_V1: &str = "https://axon.invalid/attestation/execution/v1";
...
pub enum StatementError {
    ...
    Jcs(#[from] axon_ext::jcs::JcsError),
    #[error("serialization: {0}")]
    Serde(#[from] serde_json::Error),
    #[error("dsse: {0}")]
    Dsse(#[from] dsse::DsseError),
    #[error("key: {0}")]
    Key(#[from] axon_crypto::keypair::KeyError),
