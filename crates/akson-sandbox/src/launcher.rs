...
//!
//! Akson *authors* the isolation policy as a [`SandboxSpec`]; a [`SandboxLauncher`]
...
//! boundary Akson authors, and the pure-Rust seccomp filter is handed to it via
...
//! What you write:
//! ```
//! use akson_sandbox::{SandboxSpec, NativeLauncher, Namespace};
//! let spec = SandboxSpec::clean_worker("/run/akson/task-1")
//!     .ro_bind("/opt/akson/runtime", "/runtime")   // digest-pinned, read-only
//!     .tmpfs("/scratch")
//!     .setenv("AKSON_TASK", "task-1");
...
/// The isolation policy for one worker (design §13.1). Akson builds this; a
...
/// policy Akson authors; the pure-Rust seccomp filter is handed to it via
...
mod tests {
    ...
    fn spec() -> SandboxSpec {
        SandboxSpec::clean_worker("/run/akson/task-1")
            .ro_bind("/opt/akson/runtime", "/runtime")
            .tmpfs("/scratch")
            .tmpfs("/output")
            .setenv("AKSON_TASK", "task-1")
    }
    ...
    fn the_plan_hardens_by_default() {
        ...
        assert_eq!(plan.chdir, "/run/akson/task-1");
    }
    ...
    fn live_landlock_profile_from_spec_is_enforced_in_a_child() {
        ...
        let tmp = std::env::temp_dir().join(format!("akson-llspec-{}", std::process::id()));
    ...
    fn mounts_are_ordered_proc_dev_binds_tmpfs() {
        ...
        assert_eq!(
            plan.mounts[2],
            MountOp::RoBind {
                host: "/opt/akson/runtime".to_owned(),
    ...
    fn only_declared_env_is_present_after_clearing() {
        ...
        assert_eq!(
            plan.env,
            vec![("AKSON_TASK".to_owned(), "task-1".to_owned())]
        );
    }
    ...
    fn native_launch_never_succeeds_yet() {
        ...
        let cgroup = crate::cgroup::CgroupScope::detached(std::path::PathBuf::from(
            "/sys/fs/cgroup/akson-detached-test",
        ));
    ...
    fn live_bwrap_isolates_the_worker() {
        ...
        std::env::set_var("AKSON_HOST_SECRET", "leak");
        ...
        let script = concat!(
            ...
            "[ -z \"$AKSON_HOST_SECRET\" ] || exit 21\n", // host env cleared
            "[ \"$AKSON_TASK\" = task-1 ] || exit 22\n",  // our setenv present
            ": > /scratch/ok || exit 23\n",               // scratch writable
        );
        let seccomp = crate::seccomp::SeccompPolicy::clean_worker_baseline(
            crate::seccomp::DenyAction::KillProcess,
        ...
        std::env::remove_var("AKSON_HOST_SECRET");
    ...
    fn live_confined_launch_composes_all_isolation() {
        ...
        let cgroup = CgroupScope::create(
            &format!("akson-confined-{}", std::process::id()),
            &CgroupLimits {
    ...
    fn bwrap_wires_binds_env_seccomp_and_network() {
        ...
        let ro = argv
            .windows(3)
            .any(|w| w == ["--ro-bind", "/opt/akson/runtime", "/runtime"].map(String::from));
