...
/// `AKSON_INPUT_ROOT` / `AKSON_OUTPUT_ROOT`. `adapter_cmd[0]` is the program and the
/// rest its arguments.
...
fn run_once(adapter_cmd: &[String], fixture: &Fixture) -> std::io::Result<RunResult> {
    ...
    let status = Command::new(program)
        ...
        // No AKSON_BROKER_FD: this fixture grants no processor use.
        .env_remove("AKSON_BROKER_FD")
        .status()?;
    if !status.success() {
...
fn tempdir() -> std::io::Result<std::path::PathBuf> {
    ...
    let dir = std::env::temp_dir().join(format!("akson-conformance-{}-{n}", std::process::id()));
...
mod tests {
    ...
    fn good_adapter() -> Vec<String> {
        vec![
            "/bin/sh".to_owned(),
            "-c".to_owned(),
            "head -c 32 \"$AKSON_INPUT_ROOT/diff\" > \"$AKSON_OUTPUT_ROOT/response\"".to_owned(),
        ]
    }
    ...
    fn an_over_budget_response_fails_the_budget_check() {
        ...
        let cmd = vec![
            "/bin/sh".to_owned(),
            "-c".to_owned(),
            "yes | head -c 20000 > \"$AKSON_OUTPUT_ROOT/response\"".to_owned(),
        ];
    ...
    fn writing_outside_the_response_is_caught() {
        let cmd = vec![
            "/bin/sh".to_owned(),
            "-c".to_owned(),
            "echo ok > \"$AKSON_OUTPUT_ROOT/response\"; echo leak > \"$AKSON_OUTPUT_ROOT/sneaky\""
                .to_owned(),
        ];
