...
//! What you write:
//! ```no_run
//! use akson_worker::{stage_inputs, StageItem};
//! # let dir = std::path::Path::new("/run/akson/task-1/inputs");
...
mod tests {
    ...
    fn stages_files_manifest_and_digests() {
        let dir = std::env::temp_dir().join(format!("akson-stage-{}", std::process::id()));
        let staged = stage_inputs(&items(), &dir, "/inputs").unwrap();
    ...
    fn rejects_unsafe_ids_and_duplicates() {
        let dir = std::env::temp_dir().join(format!("akson-stage-bad-{}", std::process::id()));
    ...
    fn refuses_to_overwrite_a_preexisting_target() {
        let dir = std::env::temp_dir().join(format!("akson-stage-poison-{}", std::process::id()));
