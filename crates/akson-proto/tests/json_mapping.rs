...
//! vendored definitions. These tests pin the mapping behavior Akson relies
...
use akson_proto::v1::{Message, Part, Role, Task, TaskState};
...
fn message_parses_from_a2a_json() {
    ...
    assert!(matches!(
        ...
        Some(akson_proto::v1::part::Content::Data(_))
    ));
}
...
fn unknown_standard_fields_are_ignored() {
    ...
    // benign field from a newer A2A minor does not hard-fail. Akson extension
...
fn part_oneof_is_flattened_in_json() {
    let part = Part {
        content: Some(akson_proto::v1::part::Content::Text("hi".to_owned())),
...
fn agent_card_parses_with_v1_profile_shape() {
    ...
    let card: akson_proto::v1::AgentCard = serde_json::from_str(json).unwrap();
    assert_eq!(
        card.supported_interfaces[0].protocol_version,
        akson_proto::A2A_VERSION
    );
