...
//! What you write:
//! ```
//! use axon_contract::{apply_revision, HeadState, RevisionVerdict};
//! # use axon_contract::parse_payload;
//! # use serde_json::{json, Value};
//! # fn contract(rev: u64, predecessor: Option<&str>) -> axon_contract::ParsedContract {
//! #   let mut v = json!({
//! #     "schema_version": 1, "contract_id": "00000000-0000-4000-8000-000000000000",
//! #     "revision": rev, "task_type": "https://axon.invalid/t", "message_id": "m1",
...
//! #   parse_payload(&axon_ext::jcs::canonical_bytes(&v).unwrap()).unwrap()
//! # }
...
mod tests {
    ...
    fn contract(rev: u64, predecessor: Option<&str>) -> ParsedContract {
        let mut v = json!({
            ...
            "message_id": "m1",
            "requester": {"issuer": "a", "agent": "b"},
            "performer": {"issuer": "c", "agent": "d"},
            "objective": "o",
            "inputs": [],
