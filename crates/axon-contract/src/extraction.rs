...
fn to_json(value: &axon_proto::well_known::Value) -> Option<serde_json::Value> {
    serde_json::to_value(value).ok()
}

/// Parses the contract-control Part's data value as a DSSE envelope.
fn parse_envelope(value: &axon_proto::well_known::Value) -> Result<Envelope, ExtractError> {
...
mod tests {
    use super::*;
    use axon_proto::well_known::Value as PbValue;
    use serde_json::json;
    ...
    fn sample_envelope() -> Envelope {
        Envelope {
            ...
            payload_type: "application/vnd.axon-dev.contract.v1+json".to_owned(),
            signatures: vec![axon_ext::dsse::EnvelopeSignature {
                keyid: "kid".to_owned(),
                sig: "AAAA".to_owned(),
            }],
        }
