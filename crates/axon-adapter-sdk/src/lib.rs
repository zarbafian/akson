//! The library an axon worker/adapter links to (design §16.3). An adapter runs
...
//! let mut task = Task::load()?;                         // /inputs + AXON_BROKER_FD
...
const BROKER_FD_ENV: &str = "AXON_BROKER_FD";
...
impl Task {
    ...
    /// `AXON_BROKER_FD` if `processor_use` was granted.
    ///
    /// The input/output roots default to `/inputs` and `/output` but honor
    /// `AXON_INPUT_ROOT` / `AXON_OUTPUT_ROOT`, which the conformance harness sets to
    ...
    pub fn load() -> Result<Self, Error> {
        ...
        let input_root = std::env::var("AXON_INPUT_ROOT").unwrap_or_else(|_| INPUT_ROOT.to_owned());
        let output_root =
            std::env::var("AXON_OUTPUT_ROOT").unwrap_or_else(|_| OUTPUT_ROOT.to_owned());
...
impl Broker {
    ...
    fn adopt(fd: i32) -> Result<Self, Error> {
        ...
        // worker via `AXON_BROKER_FD` (an already-connected AF_UNIX socket); the
