...
fn jcs_vectors() {
    for case in load_family("jcs") {
        ...
        let canonical = akson_ext::jcs::canonical_bytes(value)
        ...
        let digest = akson_ext::jcs::canonical_sha256(value).unwrap();
...
fn ijson_vectors() {
    ...
    for case in load_family("ijson") {
        ...
        match akson_ext::ijson::parse(&bytes) {
...
fn schema_vectors() {
    for case in load_family("schema") {
        ...
        let id = akson_ext::schema::SchemaId::by_name(schema_name, version)
        ...
        let reparsed = akson_ext::ijson::parse(&serde_json::to_vec(value).unwrap()).unwrap();
        let result = akson_ext::schema::validate(id, &reparsed);
        assert_eq!(
            result.is_ok(),
            expected_valid,
...
fn dsse_vectors() {
    for case in load_family("dsse") {
        ...
        let pae = akson_ext::dsse::pae(payload_type, payload);
        assert_eq!(
        ...
        let keyid = akson_crypto::jwk::thumbprint(&verifying_key);
        assert_eq!(
            keyid,
        ...
        let envelope = akson_ext::dsse::sign(payload_type, payload, &signing_key);
        ...
        let recovered = akson_ext::dsse::verify(&envelope, payload_type, &verifying_key).unwrap();
